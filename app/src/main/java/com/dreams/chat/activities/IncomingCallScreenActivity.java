...
//                Glide.with(this).load(user.getImage()).apply(RequestOptions.circleCropTransform().placeholder(R.drawable.ic_placeholder)).into(userImage2);
            ...
            callingType.setText(getResources().getString(R.string.app_name) + (call.getDetails().isVideoOffered() ? " Incoming Video Calling" : " Incoming Voice Calling"));
        } else {
    ...
    private void notifyMisscall(LogCall logCall) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_USER_MISSCALL, "HotRaga misscall notification", NotificationManager.IMPORTANCE_DEFAULT);
